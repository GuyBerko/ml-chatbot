...
_DEFAULT_HOST = os.getenv("HOST") or '0.0.0.0'
_DEFAULT_PORT = os.getenv("PORT") or 4002
...
def _get_and_parse(url: str) -> Dict[str, str]:
    ...
    try:
        ...
        print("** finish fetch google result duration: {} seconds **".format((datetime.now() - start).total_seconds()))
    ...
    output_dict["title"] = html.unescape(pre_rendered.renderContents().decode()) if pre_rendered else ""
...
class SearchABC(http.server.BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        for url in self.search(q=q, n=n):
            ...
            if not reason_empty_response:
                reason_content_empty = maybe_content["content"] is None or len(maybe_content["content"]) == 0
            ...
            if not any(reasons.values()):
                ...
                # Log the entry
                ###############################################################
                title_str = f"`{rich.markup.escape(maybe_content['title'])}`" if maybe_content["title"] else "<No Title>"
                print(
                ...
                # Log why it failed
                ###############################################################
                reason_string = ", ".join({reason_name for reason_name, whether_failed in reasons.items() if whether_failed})
    ...
    def search(self, q: str, n: int) -> Generator[str, None, None]:
        return NotImplemented(
            "Search is an abstract base class, not meant to be directly " "instantiated. You should instantiate a derived class like " "GoogleSearch."
        )


class GoogleSearchServer(SearchABC):
    def search(self, q: str, n: int) -> Generator[str, None, None]:
        return googlesearch.search(q, num=n, stop=None, pause=_DELAY_SEARCH, country="US")
...
class Application:
    def serve(self, host: str = _DEFAULT_HOST) -> NoReturn:
        ...
        with http.server.ThreadingHTTPServer((hostname, int(port)), GoogleSearchServer) as server:
